...
class Particlelist:
    ...
    def UpdateAccsMOND(self, EFE, shape=ball4, sigma=1, iterlen=4, regime=0):
        ...
        self.EGrav = cp.sum(cp.sqrt(1 / (cp.linalg.norm(accMONDmat, axis=0) ** 2 / a0 ** 2) + 1) * (
                    cp.linalg.norm(accMONDmat, axis=0) ** 2 / a0 ** 2)
                            + 0.5 * (cp.log(
            cp.abs(cp.sqrt(1 / (cp.linalg.norm(accMONDmat, axis=0) ** 2 / a0 ** 2) + 1) - 1))
                                     - cp.log(
                    cp.abs(cp.sqrt(1 / (cp.linalg.norm(accMONDmat, axis=0) ** 2 / a0 ** 2) + 1) + 1)))) / (
                                 8 * cp.pi * G) * cellvolume * a0 ** 2
...
class TwoBodyCircParticlelist(
    ...
    def __init__(self, m1, m2, r, phase):
        M = m1 + m2
        v = 1 / celllen * cp.sqrt(2 / 3 * cp.sqrt(G * a0 * (m1 + m2)) * (
                    1 / (1 + cp.sqrt(m1 / (m1 + m2))) + 1 / (1 + cp.sqrt(m2 / (m1 + m2)))))
...
def MainLoop(H, NDacc, func,
             EFE):  # This is the iteration loop. This calculates the MOND acceleration field from the Newtonian acceleration field. See thesis for information on why it works.
...
if simulate_two_bodies:
    ...
    #TODO: in the energy plot grav and kin energy are somehow switched.
    posmat_cuda, vecmat_cuda, AngMat_cuda, MomMat_cuda, EgravMat_cuda, EkinMat_cuda, EMat_cuda, COM_cuda = particlelist.TimeSim(
        timesteps, dt, itersteps, EFE_M, free_fall, regime)
