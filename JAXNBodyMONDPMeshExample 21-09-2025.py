...
#def AssignAccsGaussShapeJAX(accparts,accmat,particlelist2,a=1,shape=ball4):
 #   a3inv=(1/a)**3
  ...
  #  x=particlelist2[:,1]
   # y=particlelist2[:,2]
    ...
    #cellcoords= (particlelist2[:,jnp.newaxis,1].astype(int)+shape[jnp.newaxis,:,0],particlelist2[:,jnp.newaxis,2].astype(int)+shape[jnp.newaxis,:,1],particlelist2[:,jnp.newaxis,3].astype(int)+shape[jnp.newaxis,:,2])
    #weight=oversqrt2pi3*jnp.exp(-((cellcoords[0]-x[:,jnp.newaxis])**2+(cellcoords[1]-y[:,jnp.newaxis])**2+(cellcoords[2]-z[:,jnp.newaxis])**2)/(2*a**2))
    ...
    #vals_x=((accmat[0,cellcoords[0],cellcoords[1],cellcoords[2]])*weight*a3inv).ravel()
    #vals_y=((accmat[1,cellcoords[0],cellcoords[1],cellcoords[2]])*weight*a3inv).ravel()
    #vals_z=((accmat[2,cellcoords[0],cellcoords[1],cellcoords[2]])*weight*a3inv).ravel()

    #accparts=accparts.at[0,idx_x,idx_y,idx_z].add([vals_x])
    #accparts=accparts.at[1,idx_x,idx_y,idx_z].add([vals_y])
    #accparts=accparts.at[2,idx_x,idx_y,idx_z].add([vals_z])
    #return accparts
...
class Particlelist:
    ...
    def ETot(self,Epot,Egrav):
        return self.Ekin() + Epot + Egrav
    ...
    def TimeSim(self, timesteps, dt, itersteps, EFE, free_fall, regime=0):
        ...
        @jax.jit 
        def UpdateMats(t,posmat,vecmat,AngMat,MomMat,EkinMat,EgravMat,EMat,particlelist,AngMom,Ekin,Egrav,Epot):
            ...
            EMat=EMat.at[t].set(self.ETot(Egrav=Egrav,Epot=Epot))
            return posmat,vecmat,AngMat,MomMat,EkinMat,EgravMat,EMat
        ...
        for t in range(timesteps):
            ...
            posmat,vecmat,AngMat,MomMat,EkinMat,EgravMat,EMat=UpdateMats(t,posmat,vecmat,AngMat,MomMat,EkinMat,EgravMat,EMat,self.list,self.AngMom(),self.Ekin(),Egrav,Epot)
