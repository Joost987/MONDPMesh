...
def AssignMassGaussShape(density, particlelist, cellvolumeinv, a, shape):
    ...
    if tx < len(shape) and bx < len(particlelist):
        ...
        weight = (1 / math.sqrt(2 * math.pi)) ** 3 * math.exp(-(
                    ((int(x) + shape[tx][0]) - x) ** 2 + ((int(y) + shape[tx][1]) - y) ** 2 + (
                        (int(z) + shape[tx][2]) - z) ** 2) / (2 * a ** 2))
        cuda.atomic.add(density, (int(x) + shape[tx][0], int(y) + shape[tx][1], int(z) + shape[tx][2]),
...
def AssignAccsGaussShape(accparts, accmat, particlelist2, a,
                         shape):  # The same as assigning mass to the grid, but now we do not assign a mass but we assign the acceleration to the grid.
    ...
    if tx < len(shape) and ty < 3 and bx < len(particlelist2):
        ...
        weight = (1 / math.sqrt(2 * math.pi)) ** 3 * math.exp(-(
                    ((int(x) + shape[tx][0]) - x) ** 2 + ((int(y) + shape[tx][1]) - y) ** 2 + (
                        (int(z) + shape[tx][2]) - z) ** 2) / (2 * a ** 2))
        cuda.atomic.add(accparts, (bx, ty), accmat[ty][int(x) + shape[tx][0]][int(y) + shape[tx][1]][
            int(z) + shape[tx][2]] * weight * a ** (-3))
...
class TwoBodyParticlelist(Particlelist):  # Arbitary two body system
    ...
    def Analyticalacc(self):
        ...
        if regime == 0:
            ...
            V = lambda y: y**2/2 + 2/3 * y**(3/2)
            m1 = particle1[0]
            m2 = particle2[0]
            R = cp.linalg.norm(particle2[1:4] - particle1[1:4])*celllen
            Force = celllen*(particle2[1:4] - particle1[1:4])/R*a0**2*R**2/G *(V((G*m1+G*m2)/(R**2*a0)) - V(G*m1/(R**2*a0)) - V(G*m2/(R**2*a0)))
    ...
    def TimeSim(self, timesteps, dt, itersteps, EFE, free_fall, regime=0):
        ...
        posmat2 = cp.zeros([len(self.list), timesteps, 3], dtype=np.float32)
        vecmat2 = cp.zeros([len(self.list), timesteps, 3], dtype=np.float32)
        ...
        MomMat2 = cp.zeros([timesteps, 3], dtype=np.float32)
        AngMat2 = cp.zeros([timesteps, 3], dtype=np.float32)
        EMat2 = cp.zeros([timesteps], dtype=np.float32)
        EgravMat2 = cp.zeros([timesteps], dtype=np.float32)
        EkinMat2 = cp.zeros([timesteps], dtype=np.float32)
        ...
        for t in range(timesteps):
            ...
            self.list[:, 4:7] += (accold + accnew) * 0.5 * dt * cellleninv
            if t % 25 == 0: print(t)
        return posmat, vecmat, AngMat, MomMat, EMat, EkinMat, EgravMat, posmat2, vecmat2, AngMat2, MomMat2, EMat2, EkinMat2, EgravMat2, COM
...
class TwoBodyCircParticlelist(
    ...
    def __init__(self, m1, m2, r, phase):
        ...
        if regime == 0:
            ...
            R=r*celllen
            Force = a0**2*(R)**2/G *(V((G*m1+G*m2)/((R)**2*a0)) - V(G*m1/(R**2*a0)) - V(G*m2/(R**2*a0)))
            v = 1/celllen*np.sqrt(Force*R*M/(m1*m2))
        else:
            v = 1 / celllen * np.sqrt(2 / 3 * np.sqrt(G * a0 * (m1 + m2)) * (
                        1 / (1 + np.sqrt(m1 / (m1 + m2))) + 1 / (1 + np.sqrt(m2 / (m1 + m2)))))
...
if simulate_two_bodies:
   ...
   # m2, rx2, ry2, rz2, vx2, vy2, vz2 = 20, halfpixels * 9 / 8, halfpixels, halfpixels, -halfpixels, -halfpixels, 0
    ...
    #r=np.sqrt((rx1-rx2)**2+(ry1-ry2)**2+(rz1-rz2)**2)
    ...
    itersteps = 4;
    regime = 4 #TODO: regime 2 cpu/gpu
    ...
    posmat_cuda, vecmat_cuda, AngMat_cuda, MomMat_cuda, EMat_cuda, EkinMat_cuda, EgravMat_cuda, posmat2_cuda, vecmat2_cuda, AngMat2_cuda, MomMat2_cuda, EMat2_cuda, EkinMat2_cuda, EgravMat2_cuda, COM_cuda = particlelist.TimeSim(timesteps, dt, itersteps, EFE_M, free_fall, regime)
    posmat, vecmat, AngMat, MomMat, EMat, EkinMat, EgravMat, posmat2, vecmat2, AngMat2, MomMat2, EMat2, EkinMat2, EgravMat2, COM = posmat_cuda.get(), vecmat_cuda.get(), AngMat_cuda.get(), MomMat_cuda.get(), EMat_cuda.get(), EkinMat_cuda.get(), EgravMat_cuda.get(), posmat2_cuda.get(), vecmat2_cuda.get(), AngMat2_cuda.get(), MomMat2_cuda.get(), EMat2_cuda.get(), EkinMat2_cuda.get(), EgravMat2_cuda.get(), COM_cuda.get()
    ...
    plt.figure(figsize=(7, 7))
    for i in range(N_body):
        plt.plot(posmat2[i, :, 0] * size_of_box / (halfpixels * 2), posmat2[i, :, 1] * size_of_box / (halfpixels * 2),
                 label="Orbit particle " + str(i))
    plt.xlabel("$x$ (ly)");
    plt.ylabel("$y$ (ly)")
    if free_fall == 0 or free_fall == 1 or free_fall == 2:
    ...
    plt.figure(figsize=(7, 7))
    plt.plot(t_arr, EMat2, label="E total")
    plt.plot(t_arr, EkinMat2, label="E kin")
    plt.plot(t_arr, EgravMat2, label="E grav")
    plt.plot(t_arr, EMat2 - EkinMat2 - EgravMat2, label="E Pot", zorder=1)
    plt.xlabel("Time (Myr)");
    plt.ylabel("Energy")
    plt.savefig("Energy.pdf")
    plt.legend()
    ...
    for i in range(N_body):
        plt.plot(posmat2[i, :, 0] * size_of_box / (halfpixels * 2), posmat2[i, :, 1] * size_of_box / (halfpixels * 2),"--",
                    label="Orbit particle ana " + str(i))
    plt.xlabel("$x$ (ly)");
    plt.ylabel("$y$ (ly)")
    if free_fall == 0 or free_fall == 1 or free_fall == 2:
        plt.xlim(0, size_of_box);
